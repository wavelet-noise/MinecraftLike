...
{
public:
	static void bserialize(const rapidjson::Value &val)
	{
		(void)val;
	}

	template <typename Last>
	static void bserialize(const rapidjson::Value &val, const Last &last)
	{
		__bserialize(val, last.first, last.second);
	}

	template <typename First, typename... Rest>
	static void bserialize(const rapidjson::Value &val, const First &first, const Rest&... rest)
	{
		__bserialize(val, first.first, first.second);
		bserialize(val, rest...);
	}
};
...
namespace {
	template<typename _Ty>
	void __bserialize_array_part(const rapidjson::Value &val, _Ty &target)
	{
		target.JsonLoad(val);
	}

	template<typename _Ty>
	void __bserialize_array_part(const rapidjson::Value &arr, std::vector<_Ty> &target)
	{
		if (arr.IsArray())
		{
			for (decltype(arr.Size()) i = 0; i < arr.Size(); i++)
	...
	template<typename _Ty>
	void __bserialize(const rapidjson::Value &val, const char *s, _Ty &target)
	{
		if (!val.HasMember(s))
			return;
	...
	template<typename _Ty>
	void __bserialize(const rapidjson::Value &val, const char *s, std::shared_ptr<_Ty> &target)
	{
		if (!val.HasMember(s))
			return;
	...
	template<typename _Ty>
	void __bserialize(const rapidjson::Value &val, const char *s, std::vector<_Ty> &target)
	{
	...
	template<>
	void __bserialize(const rapidjson::Value &val, const char *s, int &target)
	{
		if (val.HasMember(s))
		{
			if (!val[s].IsInt())
	...
	template<>
	void __bserialize(const rapidjson::Value &val, const char *s, std::string &target)
	{
		if (val.HasMember(s))
		{
			if (!val[s].IsString())
	...
	template<>
	void __bserialize(const rapidjson::Value &val, const char *s, StringIntern &target)
	{
		if (val.HasMember(s))
		{
			if (!val[s].IsString())
	...
	template<>
	void __bserialize(const rapidjson::Value &val, const char *s, float &target)
	{
		if (val.HasMember(s))
		{
			if (!val[s].IsNumber())
	...
	template<>
	void __bserialize(const rapidjson::Value &val, const char *s, bool &target)
	{
		if (val.HasMember(s))
		{
			if (!val[s].IsBool_())
	...
	template<>
	void __bserialize(const rapidjson::Value &val, const char *s, glm::vec2 &target)
	{
		if (val.HasMember(s) && val[s].IsArray())
		{
			const rapidjson::Value &arr = val[s];
	...
	template<>
	void __bserialize(const rapidjson::Value &val, const char *s, glm::vec3 &target)
	{
		if (val.HasMember(s) && val[s].IsArray())
		{
			const rapidjson::Value &arr = val[s];
	...
	template<>
	void __bserialize(const rapidjson::Value &val, const char *s, glm::vec4 &target)
	{
		if (val.HasMember(s) && val[s].IsArray())
		{
			const rapidjson::Value &arr = val[s];
	...
	template<>
	void __bserialize(const rapidjson::Value &val, const char *s, std::vector<int> &target)
	{
		if (val.HasMember(s))
		{
			const rapidjson::Value &arr = val[s];
	...
	template<>
	void __bserialize(const rapidjson::Value &val, const char *s, std::vector<std::string> &target)
	{
		if (val.HasMember(s) && val[s].IsArray())
		{
			const rapidjson::Value &arr = val[s];
	...
	template<>
	void __bserialize(const rapidjson::Value &val, const char *s, std::vector<StringIntern> &target)
	{
		if (val.HasMember(s) && val[s].IsArray())
		{
			const rapidjson::Value &arr = val[s];
	...
	template<>
	void __bserialize(const rapidjson::Value &val, const char *s, std::vector<bool> &target)
	{
		if (val.HasMember(s) && val[s].IsArray())
		{
			const rapidjson::Value &arr = val[s];
	...
	template<>
	void __bserialize(const rapidjson::Value &val, const char *s, std::vector<float> &target)
	{
		if (val.HasMember(s) && val[s].IsArray())
		{
			const rapidjson::Value &arr = val[s];
